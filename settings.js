(function () {
  ...
  window.JigsawModules.settings=function(app){
    const KEYS={guidePicture:"jigsaw_guide_picture",guideLine:"jigsaw_guide_line",hiddenSamples:"jigsaw_hidden_samples",sampleEnabled:"jigsaw_sample_images",backupHidden:"jigsaw_hidden_samples_before_sample_off"};
    ...
    function renderSettings(uploadedCount){
      const gp=getBoolSetting(KEYS.guidePicture,true),gl=getBoolSetting(KEYS.guideLine,true);
      app.el.settingGuidePicture.querySelector("strong").textContent=gp?"ON":"OFF";app.el.settingGuideLine.querySelector("strong").textContent=gl?"ON":"OFF";
      app.el.settingGuidePicture.classList.toggle("off",!gp);app.el.settingGuideLine.classList.toggle("off",!gl);
